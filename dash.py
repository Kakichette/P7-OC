...
# Charger le modèle
model = joblib.load("data/lgbm.pkl")
#with open("data/explainer_v2.pkl", 'rb') as fs:

with open("data/explainer.pkl", 'rb') as fs:
    explainer = pickle.load(fs)
...
if isinstance(shap_values, np.ndarray):
    ...
    top_features_indices_local = np.argsort(abs(shap_values_single))[-value_max:]
...
if show_histogram:
    ...
    selected_column_neighbor = st.selectbox('Veuillez choisir une caractéristique', X.columns)
